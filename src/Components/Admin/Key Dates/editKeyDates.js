...
class editKeyDates extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loadng: true,
  ...
  getNewsDetails = async () => {
    var Keydates_item = JSON.parse(window.localStorage.getItem("Keydates_item"));
    console.log(Keydates_item, "Keydates_item");

    this.setState({
      id: Keydates_item.id,
      Name: Keydates_item.key_dates_title,
      Description: Keydates_item.key_dates_description,
     date:Keydates_item.key_dates,

     status:Keydates_item.status,
     selectedLanguage:this.state.options.find(item=>item.value==Keydates_item.language_id),
  ...
  editKeydates() {
    ...
    if (this.state.Name.length == 0) {
      ...
      this.setState({
        response: fetch("http://18.221.47.207:3000/edit_key_dates", {
          method: "POST",
          headers: {

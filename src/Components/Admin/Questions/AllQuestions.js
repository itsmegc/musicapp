...
componentDidMount(){
  ...
  getQuestions=async()=>{
    ...
    this.setState({
        loading:true
          ...
          this.setState({
          response: fetch('http://18.221.47.207:3000/get_questions', {
          method: 'GET',
          ...
          .then((responseJson)=>{
            ...
            this.setState({
                questions:responseJson.reverse(),
  ...
  if(this.state.all_selected==true ){
      ...
      return (     
        ...
        <span style={{fontFamily:'Ubuntu-B',color:'#0073aa'}} onClick={()=>this.handleEditQuestion(item)}>{item.question_title }</span>
        {/* {this.renderActions(item,index)} */}
        </div></td>
        <td>{ item.difficulty_level==1?("Easy"):(item.difficulty_level==2?"Moderate":"Tough")}</td>
...
if( item.status==2){
  return (      <tr key={item.question_id}>
      <td><input className='checkbox_content' type='checkbox' /></td>
      <td>{item.question_id}</td>
      <td style={{width:'25%'}} className="tdName textFontSize" >
      <div onMouseEnter={()=>this.showactionbutton(item,index)} onMouseLeave={()=>this.hideactionbutton()} 
...
if(this.state.item_selected){
  ...
  render() {
    if(this.state.loading){
      return(  <div className="sweet-loading-main">
      <GridLoader
        css={override}
        sizeUnit={"px"}
        size={5}
    ...
    return (
      ...
      <SidebarPage {...this.props} active={8}/>
        ...
        <div className="page_heading"><h3 style={{marginBottom:0}}>Questions</h3>
        <span onClick={()=>this.handleAddNewQuestion()} className='add_new_button'>Add New</span>
        </div> 
        <div>
    ...
    <span onClick={()=>this.setState({all_selected:false,published_selected:true,trash_selected:false,draft_selected:false})} className={this.state.published_selected ?"text_Pub current":"text_Pub"}  >Published <span className="count">({this.state.published_count})</span></span> |</li>
	<li className="draft">
    {/* <span onClick={()=>this.setState({all_selected:false,published_selected:false,trash_selected:false,draft_selected:true})} className={this.state.draft_selected ?"text_Pub current":"text_Pub"}  >Drafts <span className="count">({this.state.draft_count})</span></span> */}
     </li>
    {/* <li className="draft"> */}

...
class allLatestUpdate extends Component {
  constructor( props ) {
      super( props );
...
componentWillMount(){
      this.setState({loading:true}, ()=>this.getLatestUpdates())
...
componentDidMount(){
  ...
  getLatestUpdates=async()=>{
    ...
    this.setState({
        loading:true
          ...
          this.setState({
          response: fetch('http://18.221.47.207:3000/get_latest_updates', {
          method: 'GET',
    ...
    handleEditLatestUpdates(item){
        window.localStorage.setItem('LatestUpdates_item', JSON.stringify(item))
        window.localStorage.setItem('LatestUpdates_list',JSON.stringify(this.state.LatestUpdates))
        this.props.history.push('/admin/edit-latest-updates');
     }

     handleAddLatestUpdates(){
      window.localStorage.setItem('LatestUpdates_list',JSON.stringify(this.state.LatestUpdates))
      this.props.history.push('/admin/add-latest-updates');
   }
      ...
      renderActions(item,index){
          if(item.selected==1){
              if(this.state.trash_selected){
                ...
                return( <div  className='actions_div'><span onClick={()=>this.handleEditLatestUpdates(item,index)} 
      ...
      renderStatus(item){
        ...
        changeStatus(item,status){
            this.setState({
             ...
             this.setState({
              response: fetch('http://18.221.47.207:3000/updates_status_latest_updates', {
                method: 'POST',
                headers: {
  ...
  if(this.state.all_selected==true ){
      ...
      return (     
        ...
        <span style={{fontFamily:'OpenSans-Semibold',color:'#0073aa'}}>{item.latest_updates_title}</span>
        {this.renderActions(item,index)}
        </div></td>
        <td >   <span dangerouslySetInnerHTML={{__html:item.updates_description}} style={{fontFamily:'OpenSans-Semibold',color:'#0073aa'}}></span> 
        </td>
  ...
  return (  
    <tr key={item.id}>
      ...
      <div onMouseEnter={()=>this.showactionbutton(item,index)} onMouseLeave={()=>this.hideactionbutton()} 
      className='name_div_table'>
   <span style={{fontFamily:'OpenSans-Semibold',color:'#0073aa'}}>{item.LatestUpdates_title}</span>
      {this.renderActions(item,index)}
      </div></td>
      <td >   <span style={{fontFamily:'OpenSans-Semibold',color:'#0073aa'}}>{item.LatestUpdates_description}</span> </td>
                          <td>{this.renderActiveStatus(item)}</td>
  ...
  <GridLoader
    ...
    return (
      ...
      <SidebarPage {...this.props}  active={13} />
        ...
        <div className="page_heading"><h3 style={{marginBottom:0,marginRight:'5px'}}>All Latest Updates</h3>
        <span onClick={()=>this.handleAddLatestUpdates()} className='add_new_button'>Add New</span>
...
<span onClick={()=>console.log(this.state.selected_data)} className='apply'>Apply</span>
              ...
              <MDBTable  striped className="pages_table" style={{backgroundColor:'#F9F9F9'}}>
                  ...
                  <MDBTableBody>
                  { this.state.LatestUpdates.map((item,index) => {
...
export default allLatestUpdate;

...
componentDidMount(){
  ...
  getNews=async()=>{
    ...
    this.setState({
        loading:true
          ...
          this.setState({
          response: fetch('http://18.221.47.207:3000/get_news', {
          method: 'GET',
      ...
      renderStatus(item){
        ...
        changeStatus(item,status){
            this.setState({
             ...
             this.setState({
              response: fetch('http://18.221.47.207:3000/news_status', {
                method: 'POST',
                headers: {
  ...
  if(this.state.all_selected==true ){
      ...
      return (     
        ...
        <td >   <span style={{fontFamily:'OpenSans-Semibold',color:'#0073aa'}}>{item.news_description}</span> 
        </td>
  ...
  render() {
    ...
    return (
      ...
      <SidebarPage {...this.props}  active={13} />
        ...
        <div className="page_heading"><h3 style={{marginBottom:0,marginRight:'5px'}}>All News</h3>
        <span onClick={()=>this.handleAddNews()} className='add_new_button'>Add New</span>
        </div> 
        <div>
...
<span onClick={()=>console.log(this.state.selected_data)} className='apply'>Apply</span>
              ...
              <MDBTable  striped className="pages_table" style={{backgroundColor:'#F9F9F9'}}>
                  ...
                  {/* <th><input type="checkbox" onClick={(event)=> this.setState({item_selected:!this.state.item_selected},()=>this.handleAllChecked(event))}  checked={this.state.item_selected}  value="checkedall" /></th> */}
                       <th className="" >News Name</th>
                      <th className="textFontSize">News Description</th>
                      <th>Status</th>
                  </tr>

    ...
    getTopics = async () => {
      ...
      this.setState({
        response: fetch("http://18.221.47.207:3000/get_topics", {
          ...
          .then(response => response.json())
          .then(responseJson => {
           window.localStorage.setItem("topic_list", JSON.stringify(responseJson))
          })
          .catch(error => {
            this.setState({
              loading: false
...
/>
  ...
  render() {
    ...
    return (
      ...
      <SidebarPage {...this.props} active={3}/>
        ...
        {this.state.loading?
                    ...
                    (
             <React.Fragment>
        <div className="page_heading"><h3 style={{marginBottom:0}}>Subjects</h3>
	...
	<li className="draft">
    <span onClick={()=>this.setState({all_selected:false,published_selected:false,trash_selected:false,draft_selected:true})} className={this.state.draft_selected ?"text_Pub current":"text_Pub"}  >Drafts <span className="count">({this.state.draft_count})</span></span> </li>
    {/* <li className="draft"> */}

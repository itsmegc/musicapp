...
export default class EditQuiz extends Component {
  ...
  getQuizDetails(){
    if(!window.localStorage.getItem('quiz_item')){
        ...
        this.setState({
            ...
            //selectedSubjectTopic:this.state.subjectTopics.find(item=>item.topic_id === quizDetails.topic_id)
  ...
  getSubjects(){
      ...
      .then(responseJson => {
        ...
        this.setState({subjects:responseJson},()=>  this.getQuizDetails())
      }
      );
  ...
  getSubjectTopics(subject_id){
    ...
    for (let key of formData.entries()) {
        ...
        .then(responseJson => {
            ...
            console.log(topics)
        this.setState({subjectTopics:topics,selectedSubjectTopic:topics.find(item=>item.topic_id === this.state.quizDetails.topic_id)?topics.find(item=>item.topic_id === this.state.quizDetails.topic_id):null}, ()=>this.getTopicQuestions(this.state.selectedSubjectTopic.value))
        }
        );
  ...
  getTopicQuestions(topic_id){
    ...
    for (let key of formData.entries()) {
        ...
        console.log(this.state.quizDetails.questions)
        this.state.quizDetails.questions.forEach(question_id=>{
            if(responseJson.includes(question_id)){
                selectedTopicQuestions.push(responseJson.find(item=> item.question_id === question_id))
            }
        })
        this.setState({topicQuestions:responseJson,selectedTopicQuestions, loading:false})
        }
        );
    ...
    return (
      <div className="col-12  test-id-section d-flex align-items-center">
        ...
        <Select
          ...
          onChange={e => this.setState({ selectedSubject: e, selectedSubjectTopic:null }, ()=>this.getSubjectTopics(this.state.selectedSubject.subject_id))}

...
componentWillMount(){
      this.setState({loading:true}, ()=>this.getEvaluation())
...
getEvaluation=async()=>{
        ...
        this.setState({
            loading:true
              ...
              this.setState({
              response: fetch('http://18.221.47.207:3000/get_admin_evaluation', {
              method: 'GET',
...
//       })         
//           this.setState({
//           response: fetch('http://18.221.47.207:3000/get_tax', {
//           method: 'GET',
//           }).then((response) => response.json())
//           .then((responseJson)=>{
//               console.log(responseJson,'xxx')
...
evaluateUser=(item)=>{
    console.log(item)
    localStorage.setItem('user_evaluation',JSON.stringify(item))
    this.props.history.push(`/admin/evaluation/${item.evaluation_id}`)
}
...
//   }
  ...
  if(this.state.all_selected==true ){
      ...
      return (     
        ...
        <span style={{fontFamily:'Ubuntu-B',color:'#0073aa'}} >{item.user_name}</span>
        {/* {this.renderActions(item,index)} */}
        </div></td>
        <td className="evaluate">   <span style={{fontFamily:'Ubuntu-B',color:'#0073aa'}} onClick={()=>this.evaluateUser(item)}>Click to Evaluate</span> 
        </td>
  ...
  return (  
   ...
   <span style={{fontFamily:'Ubuntu-B',color:'#0073aa'}}>{item.user_name}</span>
      ...
      <td >   <span style={{fontFamily:'Ubuntu-B',color:'#0073aa'}}>{}</span> </td>
                          {/* <td>{this.renderActiveStatus(item)}</td> */}
  ...
  render() {
    ...
    return (
      ...
      <SidebarPage {...this.props}  active={6} />
        ...
        <div className="page_heading"><h3 style={{marginBottom:0,marginRight:'5px'}}>All Evaluations</h3>
...
<span onClick={()=>console.log(this.state.selected_data)} className='apply'>Apply</span>
              ...
              <MDBTable  striped className="pages_table" style={{backgroundColor:'#F9F9F9'}}>
                  ...
                  {/* <th><input type="checkbox" onClick={(event)=> this.setState({item_selected:!this.state.item_selected},()=>this.handleAllChecked(event))}  checked={this.state.item_selected}  value="checkedall" /></th> */}
                      ...
                      <th className="textFontSize">Evaluations</th>
                      {/* <th>Status</th> */}
                  </tr>

...
class Content extends Component {
  ...
  componentWillMount() {
    this.setState({ loading: true }, () => this.getContent());
  }
  ...
  getContent = async () => {
    ...
    this.setState({
      response: fetch("http://18.221.47.207:3000/get_content", {
        method: "GET"
      })
        ...
        .then(responseJson => {
          ...
          this.setState({
            ...
            loading: false
          },()=>console.log(this.state.content,'xxx'));
        })
        .catch(error => {
          this.setState({
            loading: false
  ...
  handleEditSubject(item) {
    window.localStorage.setItem("all_content", JSON.stringify(item));
    // window.localStorage.setItem(
    //   "subject_list",
    //   JSON.stringify(this.state.content)
    // );
    this.props.history.push("/admin/edit-content");
  }
  ...
  showactionbutton(item, index) {
    ...
    let selected_page = content[index];
    for (var v = 0; v < content.length; v++) {
      content[v].selected = 0;
    }
...
//   }
  ...
  onToggle(index, e) {
    let { content, selected_data } = this.state;
    let selected_item = content[index];
  ...
  handleAllChecked = event => {
    ...
    if (this.state.item_selected) {
      ...
      this.setState({ selected_data: content });
    } else {
      for (var v = 0; v < content.length; v++) {
        content[v].checked = 0;
      }
            ...
            {this.state.loading ? (
              ...
              <React.Fragment>
                ...
                <div className="page_heading justify-content-between">
                  <h3 style={{ marginBottom: 0 }}>Home Content</h3>
                  {/* <span
